...
The answer is a little tricky. But if you have learnt about it in class, you should have the right answer. If we use the stack data structure to implement the algorithm (note that we use a special concept 'fringe', which is similar to, but different from stack, to hold the generated nodes), the order would be like this.
